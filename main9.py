...
+ Trend score, Volatility buckets, 3x3 Microstructure Regime labels
...
# ---------------------------------------------------------
# Trend Score, Volatility, Regime functions (NEW)
# ---------------------------------------------------------

def compute_trend_score(candles: pd.DataFrame, ofi: float, prev_ofi: float) -> float:
    """
    Compute trend score from the first 10 comparison parameters.
    Returns normalized trend in [-1, 1].
    Uses weighted rules (weights chosen per earlier discussion).
    Expects 'candles' to have at least 2 rows (most recent last).
    """
    ...
    # RULE 1: Close > Prev Close
    scores.append(sgn(c['close'] - p['close'])); weights.append(1.0)

    # RULE 2: Close > Prev High (breakout) / symmetric check close < prev_low
    if c['close'] > p['high']:
        scores.append(1)
    elif c['close'] < p['low']:
        scores.append(-1)
    ...
    # RULE 3: High > Prev High (higher high) / symmetric low
    hh = 0
    if c['high'] > p['high']:
        hh = 1
    elif c['low'] < p['low']:
        hh = -1
    scores.append(hh); weights.append(1.0)

    # RULE 4: Close > Open (bullish candle)
    scores.append(sgn(c['close'] - c['open'])); weights.append(1.0)

    # RULE 5: HH & HL over last N candles (N=3)
    if len(candles) >= 3:
        prev_3_high = candles['high'].iloc[-4:-1].max() if len(candles) >= 4 else candles['high'].iloc[-3:-0].max() if len(candles) >= 3 else p['high']
        prev_3_low = candles['low'].iloc[-4:-1].min() if len(candles) >= 4 else candles['low'].iloc[-3:-0].min() if len(candles) >= 3 else p['low']
        # Simpler robust: compare last candle highs/lows vs max/min of previous 3 (excluding current)
        prev_window_high = candles['high'].iloc[-4:-1] if len(candles) >= 4 else candles['high'].iloc[:-1]
        prev_window_low = candles['low'].iloc[-4:-1] if len(candles) >= 4 else candles['low'].iloc[:-1]
        if len(prev_window_high) >= 1:
            if c['high'] > prev_window_high.max() and c['low'] > prev_window_low.min():
                scores.append(1)
            elif c['high'] < prev_window_high.max() and c['low'] < prev_window_low.min():
                scores.append(-1)
            else:
                scores.append(0)
        else:
    ...
    # RULE 6: MA short (10) > MA long (30) crossover
    closes = candles['close'].astype(float)
    ma_short = closes.rolling(window=10, min_periods=1).mean().iloc[-1]
    ma_long = closes.rolling(window=30, min_periods=1).mean().iloc[-1]
    # handle NaN fallback
    if np.isnan(ma_short):
        ma_short = closes.iloc[-1]
    if np.isnan(ma_long):
        ma_long = closes.iloc[-1]
    scores.append(sgn(ma_short - ma_long)); weights.append(1.5)

    # RULE 7: Close > MA short
    scores.append(sgn(c['close'] - ma_short)); weights.append(1.0)

    # RULE 8: OFI sign
    # use thresholding to avoid tiny OFI noise: set thr relative to scale (here 1.0)
    scores.append(sgn(ofi, thr=1e-9 if ofi == 0.0 else 0.0)); weights.append(1.5)

    # RULE 9: Volume confirmation (up-close + higher volume)
    vol_conf = 0
    if 'volume' in c and 'volume' in p and not np.isnan(c['volume']) and not np.isnan(p['volume']):
        if c['close'] > p['close'] and c['volume'] > p['volume']:
            vol_conf = 1
        elif c['close'] < p['close'] and c['volume'] > p['volume']:
            vol_conf = -1
    scores.append(vol_conf); weights.append(1.0)

    # RULE 10: Breakout magnitude: > 0.3% per candle
    prev = p['close'] if p['close'] != 0 else 1e-9
    pct = (c['close'] - prev) / float(prev)
    if pct > 0.003:
        scores.append(1)
    elif pct < -0.003:
        scores.append(-1)
    ...
    if max_possible == 0:
        return 0.0
    trend = float(total / max_possible)
    # clamp
...
def classify_volatility(vol: float) -> str:
    """
    Convert realized volatility into LOW / MEDIUM / HIGH.
    Thresholds tuned for synthetic microstructure vol.
    """
    if vol < 0.0007:
        return "LOW"
    elif vol < 0.0025:
...
def classify_regime(trend_score: float, vol_bucket: str) -> str:
    """
    Combine trend ∈ [-1,1] and vol bucket into 9-state regime.
    """
    if trend_score > 0.33:
        trend_label = "BULL"
    ...
    return f"{trend_label}-{vol_bucket}"
...
def build_regime_scatter(times_iso: List[str], ofi_vals: List[float], stress_vals: List[float]):
    """
...
def create_app(book: OrderBook, sim: MarketSimulator):
    ...
    app.layout = html.Div([
        ...
        html.Div([
            ...
            dcc.Graph(id='regime-scatter'),   # OFI vs Stress scatter (renamed)
            dcc.Graph(id='regime-timeline')   # NEW: regime timeline + current dot
    ...
    def update(n_intervals, paused, alpha_val, decay_val, ofi_store, regime_store, dataset_store):
        ...
        trend_score = 0.0
        if len(candles) >= 2:
            trend_score = compute_trend_score(candles, book.ofi, r_ofi[-2] if len(r_ofi) >= 2 else 0.0)

        # --------------------------
        # Compute Volatility
        # --------------------------
        realized_vol = sim.realized_volatility(60)
        vol_bucket = classify_volatility(realized_vol)

        # --------------------------
        # Compute Regime Label
        # --------------------------
        regime_label = classify_regime(trend_score, vol_bucket)
        r_regimes.append(regime_label)

        # trim history
        r_ts = r_ts[-1200:]
        r_ofi = r_ofi[-1200:]
        r_stress = r_stress[-1200:]
        r_regimes = r_regimes[-1200:]

        # Build OFI vs Stress scatter (regime-scatter)
        regime_scatter_fig = build_regime_scatter(r_ts, r_ofi, r_stress)

        # --------------------------
        # Build Regime Timeline Visualization (Option C)
        # --------------------------
        # Map regime -> color
        regime_colors = {
            "BULL-LOW":   "rgb(0,160,60)",
            "BULL-MEDIUM":"rgb(120,255,120)",
            "BULL-HIGH":  "rgb(255,255,0)",
            "NEUTRAL-LOW":   "rgb(120,120,120)",
            "NEUTRAL-MEDIUM":"rgb(180,180,180)",
            "NEUTRAL-HIGH":  "rgb(255,165,0)",
            "BEAR-LOW":   "rgb(255,100,100)",
            "BEAR-MEDIUM":"rgb(220,40,40)",
            "BEAR-HIGH":  "rgb(140,10,10)",
        }

        # Timeline: plot historical regimes as small markers along a line
        times_dt = pd.to_datetime(r_ts)
        colors = [regime_colors.get(x, 'white') for x in r_regimes]
        ...
        if len(times_dt) > 0:
            ...
            timeline_fig.add_trace(go.Scatter(
                ...
                hoverinfo='text',
                text=[f"{t}<br>{rg}" for t, rg in zip(r_ts, r_regimes)],
                name='Regime History'
            ))
            # Highlight the last regime with a larger circle/dot
            timeline_fig.add_trace(go.Scatter(
                x=[times_dt[-1]],
                y=[0],
                mode='markers+text',
                marker=dict(size=18, color=regime_colors.get(r_regimes[-1], 'white'), line=dict(width=1, color='white')),
                text=[r_regimes[-1]],
                textposition='top center',
                name='Current Regime'
            ))
        # style timeline
        timeline_fig.update_layout(template='plotly_dark', height=120,
        ...
        stats = [
            ...
            html.Div(f"Trend Score: {round(trend_score,3)}", style={'color':'#ccc'}),
            html.Div(f"Volatility: {vol_bucket} ({round(realized_vol,6)})", style={'color':'#ccc'}),
            html.Div(f"Regime: {regime_label}", style={'color':'#0f0', 'fontWeight':'bold'})
        ]

        # return all outputs in the same order as specified in the decorator
