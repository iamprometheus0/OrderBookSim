"""
Realistic Market Order-Book Simulator (Dash) — Heikin-Ashi aesthetic + legend fixes
- OFI + OFI timeseries
- Alpha & Decay sliders
- Clean Heikin-Ashi visuals (Binance-like colors)
Run:
...
# -----------------------
# Candle aggregation & plotting (improved HA)
# -----------------------
...
def build_candlestick_figure(candles: pd.DataFrame, trades_df: pd.DataFrame, use_heikin: bool = True):
    ...
    if not trades_df.empty:
        ...
        # unified color for trades to avoid blotchiness
        fig.add_trace(go.Scatter(
            x=df['timestamp'],
            y=df['price'],
            mode='markers',
            name='Trades',
            marker=dict(size=4, opacity=0.45, color='rgba(255,165,0,0.6)'),  # subtle orange
            showlegend=True
        ))
...
def create_app(book: OrderBook, sim: MarketSimulator):
    ...
    def update(n_intervals, paused, alpha_val, decay_val, ofi_store):
        # Ensure ofi_store is a valid dict
        ...
        return candle_fig, ofi_fig, l2_fig, stats, {'times': ts, 'values': vals}
